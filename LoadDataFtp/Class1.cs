                    ...
                    foreach (Receipt r in recepies.Values)
                    {
                        ...
                        })
                        {
                            ...
                            using (var reader = command.ExecuteReader())
                            {
                                ...
                                {
                                    long tLong;
                                    long.TryParse(reader["DOC_ID"].ToString(), out tLong);
                                    r.DocID = tLong;
                                }
                            }
                        }
                        using (var command = new SqlCommand("[dbo].[PointImport_JrnClearByDoc]", conn)
                        {
                            CommandType = CommandType.StoredProcedure
                        })
                        {
                            command.Parameters.Add("@DocID", SqlDbType.Int).Value = r.DocID;
                        }

                        foreach (RecDetail d in r.Details.Values)
                        {
                            using (var command = new SqlCommand("[dbo].[PointImport_JrnMerge]", conn)
                            {
                                CommandType = CommandType.StoredProcedure
                            })
                            {
                                command.Parameters.Add("@J_ID_XML", SqlDbType.Int).Value = d.IdXml;
                                command.Parameters.Add("@DOC_ID", SqlDbType.Int).Value = r.DocID;
                                command.Parameters.Add("@DOC_ID_XML", SqlDbType.Int).Value = r.IdXml;
                                command.Parameters.Add("@J_ENT", SqlDbType.Int).Value = d.GoodId;
                                command.Parameters.Add("@ENT_NAME", SqlDbType.NVarChar).Value = d.GoodsItemName;
                                command.Parameters.Add("@J_PRC", SqlDbType.Money).Value = d.SalePrice;
                                command.Parameters.Add("@J_QTY", SqlDbType.Money).Value = d.Quantity;
                                command.Parameters.Add("@J_SUM", SqlDbType.Money).Value = d.MoneySum;
                                command.ExecuteNonQuery();
                            }
                        }

                        using (var command = new SqlCommand("[dbo].[PointImport_SetDocImported]", conn)
                        {
                            CommandType = CommandType.StoredProcedure
                        })
                        {
